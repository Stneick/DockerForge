...
import { formatBytes, formatDuration, formatElapsed, shortId } from "@/lib/format";
...
const DockerfileView = lazy(() =>
  import("@/components/build/DockerfileView").then((m) => ({ default: m.DockerfileView })),
);
...
export function BuildDetailPage() {
  ...
  const numLabel = build ? (numbers.label(build.id) ?? shortId(build.id)) : "";
  ...
  useEffect(() => {
    if (active) {
      const t = setInterval(() => qc.invalidateQueries({ queryKey: qk.build(id, buildId) }), 5000);
      return () => clearInterval(t);
    }
  }, [active, id, buildId, qc]);
  useEffect(() => {
    if (status && !ACTIVE.includes(status)) qc.invalidateQueries({ queryKey: qk.build(id, buildId) });
  }, [status, id, buildId, qc]);

  if (buildQ.isLoading && !build) return <CenteredSpinner label="loading build…" />;
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex shrink-0 items-center gap-1.5 border-b border-line bg-bg2/60 px-4 py-2 text-xs">
        ...
        <span className="font-mono font-bold text-text">{numLabel}</span>
      ...
      <Dock
        ...
        tabs={[
          {
            id: "logs",
            label: "Logs",
            badge: active ? <StatusDot status="building" /> : undefined,
            content: (
              <LiveLogTerminal
                embedded
                logs={stream.logs}
        ...
        headerRight={
          <span className="font-mono text-2xs text-cyan">
            ⏱ {formatElapsed(build.started_at ?? build.created_at, active ? null : build.finished_at)}
          </span>
        }
      />
      ...
      <Inspector>
        <InspectorSection title={`Build ${numLabel}`}>
          <dl className="space-y-1.5 text-sm">
            <Stat k="Status" v={<StatusBadge status={status ?? build.status} />} />
            <Stat k="Duration" v={formatDuration(build.duration_seconds)} mono />
            <Stat k="Trigger" v={build.trigger_type} mono />
            <Stat k="Cache" v={build.no_cache ? "disabled" : "enabled"} mono />
            <Stat k="Size" v={build.image_size_human ?? formatBytes(build.image_size_bytes)} mono />
            <Stat k="Layers" v={build.layers?.length?.toString() ?? "—"} mono />
        ...
        <InspectorSection title="Image">
          {build.status === "success" ? (
            build.image_cleaned_at ? (
              <Banner tone="warning" icon={<Info className="h-4 w-4" />}>Image cleaned up — rebuild to download or push.</Banner>
            ) : (
              <Banner tone="success" icon={<Box className="h-4 w-4" />}>Image available.</Banner>
            )
          ) : (
            <p className="text-2xs text-dim">{active ? "Build in progress…" : "No image produced."}</p>
          )}
          <div className="mt-3">
        ...
        {build.image_tag && (
          <InspectorSection title="Tag">
            <code className="block break-all font-mono text-2xs text-muted">{build.image_tag}</code>
          </InspectorSection>
        )}
      </Inspector>
